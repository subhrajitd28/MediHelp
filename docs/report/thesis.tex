...
\newcommand{\hodname}{Dr.~[Head of Department Name]}
...
\begin{acknowledgements}
   ...
   We also thank \textbf{\hodname}, Head of \supervisordept, and the faculty
   and staff of \supervisorinstitution{} for fostering an environment in which
...
(Tesseract+Vision-LLM analysed) --- is routed through a five-stage
pipeline implemented in \texttt{run\_pipeline()}. Figure~\ref{fig:rag}
sketches the flow.
...
trigger a red modal in the Angular UI with a 108-emergency call button
and a Google Maps ``hospital near me'' link; the home-care chain
(Stage~5) is skipped to avoid downplaying the situation.
...
\paragraph{Smalltalk gate.} Above all of this, an \texttt{O}(1) prefix
gate detects greetings (``hi'', ``hello'', ``namaste'', $\ldots$), thanks
(``thanks'', ``ty'', $\ldots$), and bare ``help'' queries. These return a
canned conversational reply \textit{without} invoking any of the five
stages. The gate exists because users naturally greet the bot, and an
ungated pipeline would invent a Pharyngitis diagnosis for ``hii''.
...
\begin{figure}[H]
    ...
    \begin{minipage}[t]{0.49\linewidth}
        ...
        \subcaption*{(a) Smalltalk gate: ``hii'' returns a friendly reply
        without invoking any LLM.}
    \end{minipage}\hfill
    \begin{minipage}[t]{0.49\linewidth}
        \centering
        \includegraphics[width=\linewidth]{Images/07-chat-symptom.png}
        \subcaption*{(b) Full pipeline on a sore-throat symptom: disease,
        severity badge, nutrition table, home-care, plus cultural-food CTA.}
    \end{minipage}
    \caption{Medical-chatbot UI in two regimes --- the gate short-circuits
    on (a) and the full RAG pipeline triggers on (b).}
    \label{fig:chatbot}
\end{figure}
...
\begin{figure}[H]
    ...
    hub, FHIR export, appointment auto-completion, emergency SOS.}
    \label{fig:features}
\end{figure}
